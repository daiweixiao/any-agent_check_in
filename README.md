...
> 支持 26 个 new-api 公益站 + AnyRouter/AgentRouter 的自动签到，4 账号并行，Session 缓存加速
...
- 26 个 new-api 公益站自动签到（LinuxDO OAuth 登录）
...
脚本会自动：
1. 用缓存 session 通过 httpx 直接签到（Phase 1）
2. 对无缓存的站点启动浏览器 OAuth 登录（Phase 2）
3. 保存新 session 到缓存供下次使用
...
摸鱼公益、老魔公益站、WoW公益站、WONG公益站、KFC公益站、duckcoding黄鸭、duckcoding-jp、小呆API-base、Embedding公益站、Huan API、慕鸢公益站、佬友freestyle、New API、NPC API、Jarvis API、云端API、ibsgss公益站、星野Ai新站、Zer0by公益站、Old API、纳米哈基米

### 部分成功（4 个站点）

Einzieg API、dev88公益站、ThatAPI、Elysiver公益站（CF 验证不稳定，非固定失败）

### 跳过（7 个站点）

莹和兰、uibers、小呆API、MTU公益、略貌取神、六哥API、不知名公益站（权限不足）
...
```
.
├── multi_site_checkin.py    # 主力脚本：26 站点 x 4 账号自动签到
...
├── sessions_cache.json      # Session 缓存（自动生成）
...
**Q: 缓存文件可以删除吗？**
A: `sessions_cache.json` 删除后下次运行会重新走 OAuth（耗时更长），但不影响功能。
...
- GitHub Secrets 加密保护
- 使用私有仓库保护隐私
- `.gitignore` 防止敏感文件泄露（.env、sessions_cache.json 等）
