...
> 所有公益站均基于 **new-api** 框架（Go 后端 + React SPA 前端），使用 LinuxDO OAuth 登录。站点配置外置到 `sites.json`，运行数据存储在 `site_info.json`（程序唯一执行数据源）。目前已配置 33 个站点（27 活跃 + 6 跳过），探测发现另有多个可添加站点（详见 SITE_REGISTRY.md）。
...
│    ├── 加载 sites.json，sync_site_info() 同步到 site_info    │
│    ├── 启动 Chrome（每个 LinuxDO 账号一个实例）                  │
│    ├── Phase 1: httpx 缓存签到（site_info 中的 session）       │
...
**使用场景**：
```
工作流: probe_sites.py 探测 → 从 A 类中筛选 → 添加到 sites.json
                              ↓
...
当前已配置 27 个活跃公益站 + 6 个跳过站点，均基于 **new-api** 开源框架：
...
站点配置外置到 `sites.json`（人工维护），运行数据存储在 `site_info.json`（程序唯一执行数据源）。
...
- 移除站点 → 标记 `_removed`（数据保留，不执行）
- 跨天重置：checkin_status → pending（保留 session）
- 字段保护：note、探测结果不被覆盖

当前 27 个活跃站点 + 6 个跳过站点，完整列表见 SITE_REGISTRY.md 第四节。
...
| 标签 | 邮箱 | 说明 |
|------|------|------|
| ZHnagsan | `2621097668@qq.com` | 主号 |
| caijijiji | `dw2621097668@gmail.com` | |
| CaiWai | `daixiaowei985@gmail.com` | |
| heshangd | `2330702014@st.btbu.edu.cn` | |
...
main():
  1. 杀死现有 Chrome 进程
  2. 加载 sites.json，sync_site_info() 同步到 site_info.json
  3. resolve_sites() 补全缺失的 client_id
  4. 对 4 个账号并行执行 (asyncio.gather, 端口 9222-9225):
     - Windows: 并行模式（4 个 Chrome 实例）
     - Linux: 内存 < 3GB 自动串行（单 Chrome 实例，端口 9222），或 --serial 手动指定
     - Linux Chrome: headless + --no-sandbox + 反自动化检测

process_account(account, debug_port, info):
  === Phase 1: httpx 缓存快速签到 ===
  5. 从 site_info.json 获取该账号的活跃站点列表 (get_active_sites)
  6. 对每个有缓存 session 的站点:
     a. 检查 alive==False → 跳过（其他账号已发现站点挂了）
     b. httpx 直接 POST /api/user/checkin（携带 session cookie + New-Api-User/Authorization）
     c. 检测过期：3xx 重定向 / 401 / HTML 响应 → 清除缓存
     d. POST 404 → 自动降级为 GET 重试
     e. 站点无法连接 → 标记 alive=False（其他账号自动跳过）
     f. 成功/已签到 → 更新 site_info.json，标记为已处理
  7. 统计：缓存命中 N 个，剩余 M 个需要浏览器

  === Phase 2: 浏览器 OAuth（仅处理剩余站点）===
  8. 启动 Chrome 实例（临时 profile + CDP debug_port）
  9. Playwright 通过 CDP 连接
  ...
  11. 对每个剩余站点:
     a. 若无 client_id → 浏览器访问首页 → fetch /api/status → 获取 client_id
     ...
     k. 更新 site_info.json（session/user_id/签到状态）
     l. 记录结果到 checkin_results.json

  12. 关闭 Chrome（asyncio.wait_for 超时保护），清理临时目录
  13. 输出汇总报告（成功/失败/跳过分组）
```
...
| 串行 4 账号，有缓存（Linux 服务器） | ~10 分钟 | 4 账号依次执行 |
| 缓存全命中 | 1-2 分钟 | 仅 Phase 1 |

> 串行模式适用于阿里云等低内存 Linux 服务器（< 3GB），避免多 Chrome 实例 OOM。
...
### 6.8 最新运行结果（2026-02-16）

| 指标 | 值 |
|------|-----|
| 活跃站点 | 27 个 |
| 账号数 | 4 个（Windows 并行 / Linux 串行） |
| 总耗时（有缓存） | 189s（3.2 分钟） |
| 全部成功 | 21 个站点（4/4 账号） |
| 部分成功 | 3 个站点（CF 不稳定） |
| 站点不可达 | 3 个站点（摸鱼公益、佬友freestyle、New API） |
| 有效完成 | 90/108 (83%) |
