...
async def check_in_account(account: AccountConfig, account_index: int, app_config: AppConfig):
	...
	try:
		...
		if user_info and user_info.get('success'):
			...
			print(error_msg)
			# Check if it's an authentication failure (HTTP 401)
			if 'HTTP 401' in error_msg:
				print(f'[FAILED] {account_name}: Session expired or invalid, please update cookies')
		...
		if provider_config.needs_manual_check_in():
			...
			# Only return success if user info was retrieved successfully
			if user_info and user_info.get('success'):
				print(f'[INFO] {account_name}: Check-in completed automatically (triggered by user info request)')
				return True, user_info
			else:
				print(f'[FAILED] {account_name}: Cannot complete check-in - authentication required')
				return False, user_info
